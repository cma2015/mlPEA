...
parser$add_argument("-totalinput", help="Total input file", type="character", dest="totalinput")
parser$add_argument("-totalip", help="Total ip file", type="character", dest="totalip")
...
fIPEATransciptPeak <- function(fasta_file, totalinput, totalip, inputfile, ipfile, outputfile,fc=1.5, fdr=0.01, readsCount=10, sliding_step=25, cpus=15){
  ...
  # mapping reads number to the whole transcriptome
  mappedInput <- as.numeric(system(command= sprintf('head -n 1 %s | grep -oP "[0-9]+"',
                                                    totalinput), intern = T))
  mappedRIP <- as.numeric(system(command= sprintf('head -n 1 %s | grep -oP "[0-9]+"',
                                                  totalip), intern = T))
...
peaks_granges <- fIPEATransciptPeak(fasta_file, totalinput, totalip,inputfile, ipfile, outputfile, fc,fdr,readsCount,sliding_step, cpus)