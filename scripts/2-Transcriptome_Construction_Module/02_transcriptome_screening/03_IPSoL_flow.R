...
for(k in 1:10){
    ...
    # 如果是第一个文件，直接赋值给df_final
    if(k == 1){
        Importance_final <- df
    }else{
        # 如果不是第一个文件，按照特征名进行匹配，合并分值
...
# 为了后续的TranslationAI对转录本进行注释
#system(command = sprintf(
#  "awk '/^>/ {getline seq; print $0\":1-\"length(seq)\"(*)(0, 0)\\n\"seq}' %s/PUlearning.fa > %s/PUlearning_modified.fa",
#  output_dir,output_dir
#))
# system(command = sprintf("awk '/^>/ {print $0}' %s/PUlearning.fa | sed 's/>//g' > %s/PUlearning_ids.txt",
#                         output_dir,output_dir))
