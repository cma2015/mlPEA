...
NP = data_P.shape[0]
NU = data_U.shape[0]
#print(data_P.shape[0], data_P.shape[1],data_U.shape[0], data_U.shape[1])
...
for i in tqdm(range(T)):
    ...
    model = RandomForestClassifier(n_estimators=20, max_depth=None,max_features=None,
                                   criterion='gini', random_state=9729,class_weight='balanced',n_jobs=-1)
    # model = BaggingClassifierPU(RandomForestClassifier(n_estimators=20, random_state=9729), 
    #                      n_estimators = 20, 
    #                      n_jobs = -1
    #                      #max_samples = [500, 500]  # Each training sample will be balanced 
    #                     )
