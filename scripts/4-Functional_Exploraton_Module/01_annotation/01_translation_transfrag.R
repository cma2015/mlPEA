...
system(command = sprintf("cut -f 1,2 %s/tmp.fa_predORFs_0.5_0.5.txt > %s/tmp.fa_predORFs_0.5_0.5_cut.txt", dir, dir))
pu_translationai <- read.table(sprintf("%s/tmp.fa_predORFs_0.5_0.5_cut.txt", dir),
                                header = FALSE, sep = "\t", fill = TRUE)
pu_translationai <- pu_translationai[pu_translationai$V2 != "", ]

transcript_info <- strsplit(pu_translationai$V1, "[:,(),]")

# 提取转录本ID、起始位置、终止位置和方向信息
...
# 使orf终止位置不超过转录本长度，根据fasta序列计算transfrag长度
...
translation_command <- sprintf("/home/galaxy/miniconda3/envs/bio/bin/python /galaxy/server/tools/annotation/01_translation_transfrag.py %s %s %s",
